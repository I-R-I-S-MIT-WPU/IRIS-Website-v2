      ...
      <div className='max-w-6xl mx-auto relative z-10'>
        <div className='flex items-center justify-between mb-8'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              <h1 className='font-funnel font-bold text-2xl text-white'>
                Dashboard
              </h1>
    ...
    <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
      {stats.map((s, i) => (
        <motion.button
          ...
          <p className='text-4xl font-bold text-white font-funnel mb-1'>
            {s.value}
          </p>
      ...
      {loading ? (
        ...
        <div className='space-y-3'>
          {rows.map((row) => (
            <div
              key={row.id}
              className={`flex items-center gap-4 bg-zinc-900/80 border border-white/5 rounded-xl p-4 ${row.visible === false ? "opacity-50" : ""}`}>
              {row.image && (
                <img
                  src={row.image}
                  alt=''
              ...
              <div className='flex-grow min-w-0'>
                ...
                <p className='text-gray-500 text-xs'>
                  {row.author} &middot; {row.tag} &middot; {row.date || "—"}
                </p>
              </div>
              <div className='flex items-center gap-1 shrink-0'>
                <button
                  onClick={async () => {
                    await db.updateBlog(row.id, { pinned: !row.pinned });
                    load();
                  }}
                  className={`p-2 cursor-pointer ${row.pinned ? "text-iris-purple hover:text-iris-purple/80" : "text-gray-600 hover:text-iris-purple"}`}
                ...
                <button
                  onClick={async () => {
                    await db.updateBlog(row.id, { visible: !row.visible });
                    load();
                  }}
                  className={`p-2 cursor-pointer ${row.visible === false ? "text-gray-600 hover:text-white" : "text-emerald-400 hover:text-emerald-300"}`}
                ...
                <button
                  ...
                  className='text-gray-400 hover:text-red-400 p-2 cursor-pointer'>
...
function BlogEditor({
  ...
  const inputClass =
    "w-full bg-zinc-900 border border-white/10 focus:border-iris-purple/50 rounded-lg px-4 py-3 text-white text-sm outline-none transition-colors";

  return (
    <div>
      <button
        onClick={onCancel}
        className='flex items-center gap-1 text-gray-400 hover:text-white text-sm mb-6 cursor-pointer'>
      ...
      <form onSubmit={handleSubmit} className='max-w-3xl space-y-6'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, date: formatDate(e.target.value) }))
              }
              className={`${inputClass} cursor-pointer`}
            />
          </div>
        ...
        <div>
          ...
          <div className='flex items-center gap-3'>
            <input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, read_time: e.target.value }))
              }
            ...
            <button
              ...
              className='text-xs text-iris-purple hover:text-white border border-iris-purple/30 px-3 py-2 rounded-lg cursor-pointer transition-all'>
              Auto-estimate
            </button>
          </div>
        </div>
        ...
        <div>
          ...
          <p className='text-gray-600 text-[11px] mb-2'>
            Separate paragraphs with blank lines. Content will be rendered as
            paragraphs.
          </p>
          <textarea
            value={form.content}
        ...
        <div className='flex items-center gap-3 pt-2'>
          <button
            type='submit'
            disabled={saving}
            className='bg-iris-purple hover:bg-iris-purple/80 disabled:opacity-50 text-white px-6 py-2.5 rounded-lg text-sm font-medium cursor-pointer transition-all'>
            {saving
              ? "Publishing..."
              : initial
                ? "Update Post"
          ...
          <button
            type='button'
            onClick={onCancel}
            className='text-gray-400 hover:text-white px-4 py-2.5 text-sm cursor-pointer'>
            Cancel
          </button>
        </div>
      </form>
          ...
          {rows.map((row) => (
            <div
              ...
              <div className='flex-grow min-w-0'>
                ...
                <p className='text-gray-500 text-xs'>
                  {row.date} &middot; {row.location} &middot; {row.spots_left}{" "}
                  spots left
                </p>
              </div>
              <div className='flex items-center gap-1 shrink-0'>
                <button
                  onClick={async () => {
                    await db.updateEvent(row.id, { visible: !row.visible });
                    load();
                  }}
                  className={`p-2 cursor-pointer ${row.visible === false ? "text-gray-600 hover:text-white" : "text-emerald-400 hover:text-emerald-300"}`}
                  title={row.visible === false ? "Hidden" : "Visible"}>
...
function EventEditor({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await onSave({ ...form, spots_left: parseInt(form.spots_left) || 0 });
    } catch {
      alert("Error saving");
    }
    setSaving(false);
      ...
      <form onSubmit={handleSubmit} className='max-w-2xl space-y-5'>
        ...
        <div className='max-w-[200px]'>
          ...
          <input
            ...
            onChange={(e) =>
              setForm((p) => ({ ...p, spots_left: e.target.value }))
            }
            className={inputClass}
          />
        </div>
          ...
          {rows.map((row) => (
            <div
              ...
              <div className='flex-grow min-w-0'>
                ...
                <p className='text-gray-500 text-xs'>
                  {row.category} &middot; {row.status} &middot; Lead:{" "}
                  {row.lead || "—"}
                </p>
              </div>
              <div className='flex items-center gap-1 shrink-0'>
                <button
                  onClick={async () => {
                    await db.updateProject(row.id, { pinned: !row.pinned });
                    load();
                  }}
                  className={`p-2 cursor-pointer ${row.pinned ? "text-iris-purple hover:text-iris-purple/80" : "text-gray-600 hover:text-iris-purple"}`}
                  title={row.pinned ? "Pinned" : "Pin to top"}>
                ...
                <button
                  onClick={async () => {
                    await db.updateProject(row.id, { visible: !row.visible });
                    load();
                  }}
                  className={`p-2 cursor-pointer ${row.visible === false ? "text-gray-600 hover:text-white" : "text-emerald-400 hover:text-emerald-300"}`}
                  title={row.visible === false ? "Hidden" : "Visible"}>
      ...
      <form onSubmit={handleSubmit} className='max-w-3xl space-y-5'>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setForm((p) => ({ ...p, tech_stack: e.target.value }))
            }
            className={inputClass}
            placeholder='Comma-separated, e.g. Python, TensorFlow, ROS 2'
          />
        ...
        <div>
          ...
          <p className='text-gray-600 text-[11px] mb-3'>
            Upload images to show in the project detail page gallery
          </p>
          {gallery.length > 0 && (
        ...
        <div>
          ...
          <p className='text-gray-600 text-[11px] mb-3'>
            Select members working on this project
          </p>
          {allMembers.length === 0 ? (
            <p className='text-gray-500 text-xs'>
              No members in database yet. Add members first.
            </p>
          ) : (
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[240px] overflow-y-auto pr-2'>
              {allMembers.map((m) => (
                <button
                  ...
                  {teamMembers.includes(m.id) && (
                    <span className='ml-auto text-iris-purple text-xs font-bold shrink-0'>
                      ✓
                    </span>
                  )}
                </button>
            ...
            <div
              ...
              <div className='flex-grow min-w-0'>
                ...
                <p className='text-gray-500 text-xs truncate'>
                  {row.position} &middot; {row.domain}
                </p>
              </div>
              <div className='flex items-center gap-0.5 shrink-0'>
                ...
                <button
                  onClick={() => setEditing(row)}
                  className='text-gray-400 hover:text-white p-1.5 cursor-pointer'>
          ...
          {rows.map((row) => (
            <div
              ...
              <div className='flex-grow min-w-0'>
                ...
                <p className='text-gray-500 text-xs'>
                  {row.title} &middot; {row.department}
                </p>
              </div>
              <div className='flex items-center gap-1 shrink-0'>
                <button
                  onClick={async () => {
                    await db.updateMentor(row.id, { visible: !row.visible });
                    load();
                  }}
                  className={`p-2 cursor-pointer ${row.visible === false ? "text-gray-600 hover:text-white" : "text-emerald-400 hover:text-emerald-300"}`}
                  title={row.visible === false ? "Hidden" : "Visible"}>
...
function MentorEditor({
  ...
  return (
    <div>
      ...
      <h2 className='text-white font-semibold text-lg mb-6'>
        {initial ? "Edit Mentor" : "Add Mentor"}
      </h2>
      <form onSubmit={handleSubmit} className='max-w-2xl space-y-5'>
        {/* Photo */}
        <div>
...
function CrudTable({
  ...
  const load = () => {
    setLoading(true);
    const fn = (db as any)[
      `get${table.charAt(0).toUpperCase() + table.slice(1)}`
    ];
    fn()
      .then((d: any) => {
        setRows(d || []);
  ...
  const handleDelete = async (id: string) => {
    ...
    const fn = (db as any)[
      `delete${table.charAt(0).toUpperCase() + table.slice(1).replace(/s$/, "")}`
    ];
    await fn(id);
    load();
  };
  ...
  const handleSave = async (data: Record<string, any>) => {
    const entity =
      table.charAt(0).toUpperCase() + table.slice(1).replace(/s$/, "");
            ...
            <thead>
              <tr className='border-b border-white/10'>
                {columns.map((c) => (
                  <th
                    key={c}
                    className='text-left text-gray-500 text-xs uppercase tracking-wider py-3 px-3'>
                    {c.replace(/_/g, " ")}
                  </th>
                ))}
                <th className='text-right py-3 px-3 text-gray-500 text-xs uppercase tracking-wider'>
                  Actions
                </th>
              </tr>
            </thead>
              ...
              {rows.map((row) => (
                <tr
                  ...
                  {columns.map((c) => (
                    <td
                      key={c}
                      className='py-3 px-3 text-gray-300 max-w-[200px] truncate'>
                      {Array.isArray(row[c])
                        ? row[c].join(", ")
                        : row[c] || "—"}
                    </td>
                  ...
                  <td className='py-3 px-3 text-right'>
                    ...
                    <button
                      onClick={() => handleDelete(row.id)}
                      className='text-gray-400 hover:text-red-400 p-1.5 cursor-pointer ml-1'>
                      <Trash2 className='w-4 h-4' />
                    </button>
                  </td>
                </tr>
          ...
          <div key={f}>
            ...
            f === "bio" ? (
              ...
              <div className='flex items-start gap-3'>
                <div className='flex-grow'>
                  ...
                  <button
                    ...
                    onClick={() => fileRef.current?.click()}
                    disabled={uploading}
                    className='mt-2 flex items-center gap-2 text-xs text-iris-purple hover:text-white border border-iris-purple/30 px-3 py-1.5 rounded-lg cursor-pointer transition-all disabled:opacity-50'>
              ...
              <input
                type='text'
                value={
                  Array.isArray(form[f]) ? form[f].join(", ") : form[f] || ""
                }
                onChange={(e) =>
                  setForm((p) => ({ ...p, [f]: e.target.value }))
                }
    ...
    <div>
      <div className='flex items-center justify-between mb-6'>
        ...
        <button
          onClick={() => fileRef.current?.click()}
          disabled={uploading}
          className='flex items-center gap-2 bg-iris-purple hover:bg-iris-purple/80 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-medium cursor-pointer transition-all'>
          <Upload className='w-4 h-4' />{" "}
      ...
      {loading ? (
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4'>
          {rows.map((row) => (
            <div
              key={row.id}
              className={`relative group rounded-xl overflow-hidden border border-white/5 ${row.visible === false ? "opacity-50" : ""}`}>
              <img
                src={row.image}
                alt=''
                referrerPolicy='no-referrer'
              ...
              <div className='absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2'>
                <button
                  onClick={async () => {
                    await db.updateGalleryImage(row.id, {
                      visible: !row.visible,
                    });
                    load();
                  }}
                  className={`p-2 rounded-lg cursor-pointer ${row.visible === false ? "bg-gray-800 text-gray-400 hover:text-white" : "bg-emerald-600/20 text-emerald-400 hover:text-emerald-300"}`}>
                  {row.visible === false ? (
                    <EyeOff className='w-4 h-4' />
                  ) : (
                    <Eye className='w-4 h-4' />
                ...
                <button
                  onClick={() => handleDelete(row.id)}
                  className='p-2 bg-red-600/20 text-red-400 hover:text-red-300 rounded-lg cursor-pointer'>
                  <Trash2 className='w-4 h-4' />
                </button>
              </div>
            </div>
              ...
              <div
                ...
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <p className='text-gray-500 text-xs'>
                      {app.email} &middot; {app.branch} &middot; Year {app.year}
                    </p>
                  </div>
          ...
          <div>
            ...
            <div className='space-y-3 text-sm'>
              <div>
                ...
                <span className='text-white ml-2'>{selected.prn}</span>
              </div>
              <div>
                <span className='text-gray-500'>Email:</span>{" "}
                <span className='text-white ml-2'>{selected.email}</span>
              </div>
              <div>
                <span className='text-gray-500'>Phone:</span>{" "}
                <span className='text-white ml-2'>{selected.phone}</span>
              </div>
              <div>
                <span className='text-gray-500'>Branch:</span>{" "}
                <span className='text-white ml-2'>{selected.branch}</span>
              </div>
              <div>
                <span className='text-gray-500'>Year:</span>{" "}
                <span className='text-white ml-2'>{selected.year}</span>
              </div>
              <div>
              ...
              {selected.why && (
                <div>
                  <span className='text-gray-500'>Why IRIS:</span>{" "}
                  <span className='text-gray-300 ml-2'>{selected.why}</span>
                </div>
              )}
              {selected.resume_url && (
                <a
                  ...
                  className='text-iris-purple text-xs hover:underline block mt-2'>
                  View Resume
                </a>
              )}
            </div>
            <div className='flex gap-2 mt-6'>
              <button
                ...
                className='bg-emerald-600 hover:bg-emerald-500 text-white text-xs px-3 py-1.5 rounded-lg cursor-pointer'>
                Accept
              </button>
              <button
                onClick={() => handleStatus(selected.id, "rejected")}
                className='bg-red-600 hover:bg-red-500 text-white text-xs px-3 py-1.5 rounded-lg cursor-pointer'>
                Reject
              </button>
              <button
                onClick={() => handleStatus(selected.id, "pending")}
                className='bg-zinc-700 hover:bg-zinc-600 text-white text-xs px-3 py-1.5 rounded-lg cursor-pointer'>
                Pending
              </button>
            </div>
          </div>
