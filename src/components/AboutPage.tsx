...
import { MENTORS as FALLBACK_MENTORS, MEMBERS as FALLBACK_MEMBERS, LOCATIONS } from '../data/team';
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
          ...
          <p className="text-gray-400 text-base sm:text-lg leading-relaxed font-sans max-w-3xl">
            I.R.I.S (Innovation, Research, Intelligence & Support) is a student-run technical club at MIT-WPU, Pune. We work across robotics, AI/ML, avionics, full-stack engineering, and design — shipping real products, not just prototypes.
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
            {members.map(member => (
              ...
              >
                <img src={member.photo} alt={member.name} referrerPolicy="no-referrer" className="w-20 h-20 rounded-full object-cover mx-auto mb-3 group-hover:ring-2 group-hover:ring-iris-purple/50 transition-all" />
      ...
      <AnimatePresence>
        {selectedMember && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelectedMember(null)} className="absolute inset-0 bg-black/80 backdrop-blur-sm" />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-sm bg-zinc-950 border border-white/10 rounded-2xl p-8 text-center z-10">
              <button onClick={() => setSelectedMember(null)} className="absolute top-4 right-4 text-gray-400 hover:text-white cursor-pointer"><X className="w-5 h-5" /></button>
              ...
              <div className="flex items-center justify-center gap-3">
                {selectedMember.linkedin && <a href={selectedMember.linkedin} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:border-iris-purple/40 transition-all"><Linkedin className="w-4 h-4" /></a>}
                {selectedMember.github && <a href={selectedMember.github} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:border-iris-purple/40 transition-all"><Github className="w-4 h-4" /></a>}
                {selectedMember.instagram && <a href={selectedMember.instagram} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:border-iris-purple/40 transition-all"><Instagram className="w-4 h-4" /></a>}
              </div>
            </motion.div>
          </div>
        )}
